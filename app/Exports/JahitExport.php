    ...
    public function registerEvents(): array
    {
        $jahits = RiwayatJahit::query()
            ...
            ->when($this->kategori, function ($query) {
                $query->where('jahit.status_id', $this->kategori);
            })
            ...
            AfterSheet::class => function (AfterSheet $event) use ($jahits) {
                ...
                if ($jahits->isEmpty()) {
                    ...
                    foreach ($jahits as $index => $jahit) {
                        ...
                        $sheet->setCellValue("B{$row}", $jahit->jahit->order_id ?? '-');
                        $sheet->setCellValue("C{$row}", $jahit->jahit->status->nama ?? '-');
                        $sheet->setCellValue("D{$row}", $jahit->user_nama ?? '-');
                        $sheet->setCellValue("E{$row}", $jahit->jumlah_dikerjakan);
                        $sheet->setCellValue("F{$row}", \Carbon\Carbon::parse($jahit->created_at)->format('d-m-Y'));
                        $row++;
                    }
                }
