  ...
  if (invalidFiles.length > 0) {
    const messages = invalidFiles.map(file => {
      if (!file.type.startsWith('image/')) {
        ...
        return `File "${file.name}" melebihi 5MB.`;
      }
      return '';
    });

    // Tampilkan 1 toast saja (gabungan pesan atau ambil yang pertama)
    toast.error(messages[0]); // atau join jika ingin banyak
    target.value = ''; // reset input
    return;
  }
  form.foto_produk = files;
  form.is_cover = null;
}
...
function submit() {
  ...
  form.post(route('data.produk.store'), {
    replace: true,
    forceFormData: true,
    onSuccess: () => {
    },
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div>
              <label class="block mb-1 font-medium">Kategori</label>
                <Select
                    ...
                    :disabled="props.kategoris.length === 0"
                    class="w-full"
                />
            ...
            <div>
              <label class="block mb-1 font-medium">Satuan</label>
                <Select
                    ...
                    :disabled="props.satuans.length === 0"
                    class="w-full"
                />
          ...
          <div>
            <label class="block mb-1 font-medium">Foto Produk (bisa lebih dari satu)</label>
            <input
                type="file"
                multiple
                accept="image/*"
