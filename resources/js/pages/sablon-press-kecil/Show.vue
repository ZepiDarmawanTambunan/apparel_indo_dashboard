...
const fileSablonPress = ref<HTMLInputElement | null>(null);
const fotoKerusakan = ref<HTMLInputElement | null>(null);

//  START HANDLE SABLON PRESS
const formSablonPress = useForm<{
    _method: string,
    user_id: number | null,
    keterangan: string | null,
    file_sablon_press: File | null,
}>({
  _method: 'PUT',
  user_id: props.sablon_press.user_id ?? null,
  keterangan: props.sablon_press.keterangan ?? '',
  file_sablon_press: null,
});

const submitSablonPress = () => {
    const data = new FormData();
    data.append('_method', 'PUT');
    data.append('user_id', String(formSablonPress.user_id));
    data.append('keterangan', formSablonPress.keterangan ?? '');
    if (formSablonPress.file_sablon_press) {
        data.append('file_sablon_press', formSablonPress.file_sablon_press);
    }
    formSablonPress.post(route('sablon-press-kecil.update', props.sablon_press.id), {
        forceFormData: true,
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            formSablonPress.reset('file_sablon_press');
            if (fileSablonPress.value) fileSablonPress.value.value = '';
            formSablonPress.clearErrors();
        },
        onError: (errors) => console.log(errors),
    });
};
// END HANDLE SABLON PRESS

//  START HANDLE FOTO SABLON PRESS
function onFileChangeSablonPress(event: Event) {
  ...
  if (target.files && target.files[0]) {
    ...
    formSablonPress.file_sablon_press = file;
  }
}
...
// END HANDLE FOTO SABLON PRESS

// START HANDLE TOMBOL SELESAI DAN BATAL
const confirm = useConfirm();
    const tombolSelesai = () => {
        confirm.require({
        message: 'Yakin ingin menandai sablon press sebagai selesai?',
        header: 'Konfirmasi',
        acceptLabel: 'Ya',
        rejectLabel: 'Batal',
        acceptClass: 'p-button-danger',
        accept: () => {
                router.put(route('sablon-press-kecil.selesai', props.sablon_press.id), {}, {
                preserveScroll: true,
                onError: () => toast.error('Gagal menyelesaikan sablon press'),
            });
        },
    });
};
...
const tombolBatal = () => {
    confirm.require({
        message: 'Yakin ingin membatalkan sablon press ini?',
        header: 'Konfirmasi',
        acceptLabel: 'Ya',
        rejectLabel: 'Batal',
        acceptClass: 'p-button-danger',
        accept: () => {
                router.put(route('sablon-press-kecil.batal', props.sablon_press.id), {}, {
                preserveScroll: true,
                onError: () => toast.error('Gagal membatalkan sablon press'),
            });
        },
    });
};
...
const formKerusakan = useForm({
  order_id: props.sablon_press.order_id,
  keterangan: '',
  divisi_pelapor: 'Sablon Press',
  jumlah_rusak: 1,
...
<template>
    <Head title="Detail Sablon Press" />
    <AppLayout>
        <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl p-6 shadow hover:shadow-lg transition">
                <h2 class="text-2xl font-semibold mb-6">Detail Sablon Press</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 font-medium">Order ID</label>
                        <input :value="sablon_press.order?.id_order" readonly class="w-full border rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Nama Pelanggan</label>
                        <input :value="sablon_press.order?.nama_pelanggan" readonly class="w-full border rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Nomor WA</label>
                        <input :value="sablon_press.order?.nohp_wa" readonly class="w-full border rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Status Sablon Press</label>
                        <input :value="sablon_press.status?.nama ?? '-'" readonly class="w-full border rounded px-3 py-2" />
                    </div>
                </div>
                <div class="mt-6 flex justify-end">
                    <Link
                    v-if="sablon_press.order"
                    :href="route('order.show', sablon_press.order_id)"
            ...
            <div class="bg-white rounded-xl p-6 mt-5 shadow hover:shadow-lg transition">
                <h2 class="text-2xl font-semibold mb-6">Input Sablon Press</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 font-medium">User</label>
                        <Select
                            v-model="formSablonPress.user_id"
                            ...
                            class="w-full"
                        />
                        <span v-if="formSablonPress.errors.user_id" class="text-red-500 text-sm">
                            {{ formSablonPress.errors.user_id }}
                        </span>
                    </div>
                    <div>
                        <label class="block mb-1 font-medium">Keterangan</label>
                        <textarea v-model="formSablonPress.keterangan" class="w-full border rounded px-3 py-2" rows="3" />
                    </div>
                    <div>
                        ...
                        <input
                            type="file"
                            accept="image/*"
                            @change="onFileChangeSablonPress"
                        ...
                        />
                        <span v-if="formSablonPress.errors.file_sablon_press" class="text-red-500 text-sm">
                            {{ formSablonPress.errors.file_sablon_press }}
                        </span>
                    </div>
                    <div v-if="formSablonPress.file_sablon_press">
                        <p class="font-medium mb-2">Preview Gambar:</p>
                        <img :src="getObjectURL(formSablonPress.file_sablon_press)" class="w-full max-h-64 object-contain border rounded" />
                    </div>
                </div>

                <div class="mt-6 flex justify-end gap-2">
                    <a
                        v-if="sablon_press.file_sablon_press"
                        :href="sablon_press.file_sablon_press"
                        target="_blank"
                    ...
                    <button
                        v-if="!['Selesai', 'Batal'].includes(sablon_press.status?.nama ?? '')"
                        @click="submitSablonPress"
                        type="button"
                        class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition cursor-pointer"
                        :disabled="formSablonPress.processing"
                    >
                        Simpan
                    </button>
                    <button
                    ...
                    <button
                        v-if="['Selesai'].includes(sablon_press.status?.nama ?? '')"
                ...
                <DataTable
                    ...
                    <Column header="Aksi">
                        <template #body="slotProps">
                                ...
                                <!-- <button
                                    v-if="['Selesai', 'Proses'].includes(slotProps.data.status.nama) && sablon_press.status?.nama == 'Proses' && slotProps.data.divisi_pelapor == 'Sablon Press'"
                                ...
                                <button
                                    v-if="slotProps.data.status.nama == 'Proses' && sablon_press.status?.nama == 'Proses' && slotProps.data.divisi_pelapor == 'Sablon Press'"
