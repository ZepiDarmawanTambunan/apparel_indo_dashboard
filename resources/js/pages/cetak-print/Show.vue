...
const fileCetakPrint = ref<HTMLInputElement | null>(null);
const fotoKerusakan = ref<HTMLInputElement | null>(null);

//  START HANDLE CETAK PRINT
const formCetakPrint = useForm({
  _method: 'PUT',
  user_id: props.cetak_print.user_id,
  keterangan: props.cetak_print.keterangan || '',
  file_cetak_print: null as File | null,
});

const submitCetakPrint = () => {
    const data = new FormData();
    data.append('_method', 'PUT');
    data.append('user_id', String(formCetakPrint.user_id));
    data.append('keterangan', formCetakPrint.keterangan);
    if (formCetakPrint.file_cetak_print) {
        data.append('file_cetak_print', formCetakPrint.file_cetak_print);
    }
    formCetakPrint.post(route('cetak-print.update', props.cetak_print.id), {
        forceFormData: true,
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            formCetakPrint.reset('file_cetak_print');
            if (fileCetakPrint.value) fileCetakPrint.value.value = '';
            formCetakPrint.clearErrors();
        },
        onError: (errors) => console.log(errors),
    });
};
// END HANDLE CETAK PRINT

//  START HANDLE FOTO CETAK PRINT
function onFileChangeCetakPrint(event: Event) {
  ...
  if (target.files && target.files[0]) {
    ...
    formCetakPrint.file_cetak_print = file;
  }
}
...
// END HANDLE FOTO CETAK PRINT

// START HANDLE TOMBOL SELESAI DAN BATAL CETAK PRINT
const confirm = useConfirm();
    const tombolSelesai = () => {
        confirm.require({
        message: 'Yakin ingin menandai cetak print sebagai selesai?',
        header: 'Konfirmasi',
        acceptLabel: 'Ya',
        rejectLabel: 'Batal',
        acceptClass: 'p-button-danger',
        accept: () => {
                router.put(route('cetak-print.selesai', props.cetak_print.id), {}, {
                preserveScroll: true,
                onError: () => toast.error('Gagal menyelesaikan Cetak Print'),
            });
        },
    });
};
...
const tombolBatal = () => {
    confirm.require({
        message: 'Yakin ingin membatalkan cetak print ini?',
        header: 'Konfirmasi',
        acceptLabel: 'Ya',
        rejectLabel: 'Batal',
        acceptClass: 'p-button-danger',
        accept: () => {
                router.put(route('cetak-print.batal', props.cetak_print.id), {}, {
                preserveScroll: true,
                onError: () => toast.error('Gagal membatalkan Cetak Print'),
            });
        },
    });
};
...
const formKerusakan = useForm({
  order_id: props.cetak_print.order_id,
  keterangan: '',
  divisi_pelapor: 'Cetak Print',
  jumlah_rusak: 1,
...
<template>
    <Head title="Detail Cetak Print" />
    <AppLayout>
        <div class="py-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl p-6 shadow hover:shadow-lg transition">
            <h2 class="text-2xl font-semibold mb-6">Detail Cetak Print</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 font-medium">Order ID</label>
                        <input :value="cetak_print.order?.id_order" readonly class="w-full border rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Nama Pelanggan</label>
                        <input :value="cetak_print.order?.nama_pelanggan" readonly class="w-full border rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Nomor WA</label>
                        <input :value="cetak_print.order?.nohp_wa" readonly class="w-full border rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Status Cetak Print</label>
                        <input :value="cetak_print.status?.nama ?? '-'" readonly class="w-full border rounded px-3 py-2" />
                    </div>
                </div>
                <div class="mt-6 flex justify-end">
                    <Link
                        v-if="cetak_print.order"
                        :href="route('order.show', cetak_print.order_id)"
            ...
            <div class="bg-white rounded-xl p-6 mt-5 shadow hover:shadow-lg transition">
                <h2 class="text-2xl font-semibold mb-6">Input Cetak Print</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 font-medium">User</label>
                        <Select
                            v-model="formCetakPrint.user_id"
                            ...
                            class="w-full"
                        />
                        <span v-if="formCetakPrint.errors.user_id" class="text-red-500 text-sm">
                            {{ formCetakPrint.errors.user_id }}
                        </span>
                    </div>
                    <div>
                        <label class="block mb-1 font-medium">Keterangan</label>
                        <textarea v-model="formCetakPrint.keterangan" class="w-full border rounded px-3 py-2" rows="3" />
                    </div>
                    <div>
                        ...
                        />
                        <span v-if="formCetakPrint.errors.file_cetak_print" class="text-red-500 text-sm">
                            {{ formCetakPrint.errors.file_cetak_print }}
                        </span>
                    </div>
                    <div v-if="formCetakPrint.file_cetak_print">
                        <p class="font-medium mb-2">Preview Gambar:</p>
                        <img :src="getObjectURL(formCetakPrint.file_cetak_print)" class="w-full max-h-64 object-contain border rounded" />
                    </div>
                </div>

                <div class="mt-6 flex justify-end gap-2">
                    <a
                        v-if="cetak_print.file_cetak_print"
                        :href="cetak_print.file_cetak_print"
                        target="_blank"
                    ...
                    <button
                        v-if="!['Selesai', 'Batal'].includes(cetak_print.status?.nama ?? '')"
                        @click="submitCetakPrint"
                        type="button"
                        class="px-4 py-2 bg-indigo-600 text-sm font-medium text-white rounded hover:bg-indigo-700 transition cursor-pointer"
                        :disabled="formCetakPrint.processing"
                    >
                        Simpan
                    </button>
                    <button
                        ...
                        @click="tombolSelesai"
                        type="button"
                        class="px-4 py-2 bg-green-600 text-sm font-medium text-white rounded hover:bg-green-700 transition cursor-pointer"
                        :disabled="formCetakPrint.processing"
                    >
                        Selesai
                    </button>
                    <button
                        v-if="['Selesai'].includes(cetak_print.status?.nama ?? '')"
                        @click.stop="tombolBatal"
                        class="px-4 py-2 rounded bg-red-600 text-sm font-medium text-white hover:bg-red-700 transition cursor-pointer"
                        :disabled="formCetakPrint.processing"
                    >
                        Batal
                    </button>
            ...
            <div class="bg-white rounded-xl p-6 mt-5 shadow hover:shadow-lg transition">
                ...
                <div
                    ...
                    <button @click="submitKerusakan" type="button" class="px-4 py-2 text-sm font-medium bg-indigo-600 text-white rounded hover:bg-indigo-700 transition cursor-pointer" :disabled="formKerusakan.processing">
                        Simpan
                    </button>
                </div>
            </div>
                ...
                <DataTable
                    ...
                    <Column header="Aksi">
                        <template #body="slotProps">
                                ...
                                <!-- <button
                                    v-if="['Selesai', 'Proses'].includes(slotProps.data.status.nama) && cetak_print.status?.nama == 'Proses'"
                                    @click="batalLaporan(slotProps.data.id)"
                                    class="px-3 py-2 text-sm font-medium rounded bg-red-500 text-white hover:bg-red-600 cursor-pointer"
                                >
                                    Batal
                                </button> -->
                                <!-- <button
                                    v-if="slotProps.data.status.nama == 'Proses' && cetak_print.status?.nama == 'Proses'"
                                    @click="selesaiLaporan(slotProps.data.id)"
                                    class="px-3 py-2 text-sm font-medium rounded bg-green-500 text-white hover:bg-green-600 cursor-pointer"
                                >
                                    Selesai
                                </button> -->
                            </div>
