...
const filteredQc = computed(() => {
  if (!search.value) return qc;
...
const onRowClick = (event: any) => {
  const qc = event.data;
  router.get(route('qc.show', qc.id));
};

const confirm = useConfirm();
const batalQc = (id: string) => {
    confirm.require({
        message: 'Yakin ingin membatalkan data qc ini?',
        header: 'Konfirmasi',
        acceptLabel: 'Ya',
        rejectLabel: 'Batal',
        acceptClass: 'p-button-danger',
        accept: () => {
            router.put(route('qc.batal', id), {}, {
                preserveScroll: true,
                preserveState: false,
                onSuccess: () => {},
                onError: () => toast.error('Gagal membatalkan data qc'),
            });
        },
    });
};
        ...
        <div class="mb-6 space-y-4">
          ...
          <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            ...
            <input
                type="text"
                v-model="search"
                placeholder="Cari quality control..."
        ...
        >
          <template #empty>
            <div class="flex items-center justify-center py-10 text-gray-400">
              Tidak ada data qc yang tersedia.
            </div>
          </template>
          ...
          <Column header="Aksi" :style="{ width: '260px' }">
            <template #body="{ data }">
                <button
                  ...
                  @click.stop="batalQc(data.id)"
                  class="px-3 py-2 rounded font-medium bg-red-600 text-white hover:bg-red-700 text-sm cursor-pointer"
                  title="Batal"
                >
                  Batal
                </button>
